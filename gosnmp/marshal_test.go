			...
			t.Run("unmarshal", func(t *testing.T) {
				...
				for n, vb := range test.out.Variables {
					...
					case OpaqueUinteger64:
						if vb.Value.(uint64) != vbr.Value.(uint64) {
							t.Errorf("#%d:%d Value result: %v, test: %v", i, n, vbr.Value, vb.Value)
						}
					default:
...
func opaqueUint64Response() []byte {
	return []byte{
		...
		0x7b, 0x01, 0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	}
}
