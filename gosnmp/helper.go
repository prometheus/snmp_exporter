...
func marshalUint64OrNil(v interface{}) ([]byte, error) {
	if v == nil {
...
// TODO: add OpaqueCounter64 (0x76), OpaqueInteger64 (0x7a)
func parseOpaque(logger Logger, data []byte, retVal *variable) error {
	if len(data) == 0 {
		return ErrZeroByteBuffer
	}
	logger.Printf("Raw Opaque: %v", data)
	if len(data) > 2 && data[0] == AsnExtensionTag {
		...
		case OpaqueUinteger64:
			// 0x7B
			data = data[1:]
			logger.Print("decodeValue: type is OpaqueUinteger64")
			length, cursor, err := parseLength(data)
			if err != nil {
				return err
			}
			if length > len(data) {
				return fmt.Errorf("not enough data for OpaqueUinteger64 %x (data %d length %d)", data, len(data), length)
			}
			retVal.Type = OpaqueUinteger64
			retVal.Value, err = parseUint64(data[cursor:length])
			if err != nil {
				return err
			}
		default:
